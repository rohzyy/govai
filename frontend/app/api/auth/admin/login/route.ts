...
export async function POST(request: NextRequest) {
    try {
        ...
        try {
            const response = await backendAPI.post('/auth/admin/login', body);
            if (response && response.data) {
                backendData = response.data;
                cookies = response.headers['set-cookie'];
            }
        } catch (apiError: any) {
            console.error('[POST /api/auth/admin/login] Backend error:', apiError?.message);

            const backendMessage = apiError?.response?.data?.detail;
            return NextResponse.json({
                success: false,
                error: 'AUTH_FAILED',
                safeMessage: backendMessage || 'Admin login failed.',
                data: null,
                meta: { healthy: false }
            });
        }
        ...
        console.error('[POST /api/auth/admin/login] FATAL:', fatalError);
        return NextResponse.json({
            success: false,
