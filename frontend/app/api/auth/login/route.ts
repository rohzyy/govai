...
export async function POST(request: NextRequest) {
    try {
        ...
        try {
            ...
            console.error('[POST /api/auth/login] Backend error:', apiError?.message);

            const backendMessage = apiError?.response?.data?.detail;
            return NextResponse.json({
                success: false,
                error: 'AUTH_FAILED',
                safeMessage: backendMessage || 'Login failed. Please check credentials.',
                data: null,
                meta: { healthy: false }
            });
        }
        ...
        console.error('[POST /api/auth/login] FATAL:', fatalError);
        return NextResponse.json({
            success: false,
